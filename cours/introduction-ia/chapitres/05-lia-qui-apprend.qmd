...
numero: "6"
description: "Séance 5 d'Introduction à l'IA : utilité espérée maximale, processus de décision markoviens, équation de Bellman, value iteration, Q-learning et descente de gradient."
...
En Séance 4, nous avons résolu l'un des problèmes fondamentaux de l'IA : *comment raisonner sous incertitude*. Bayes et les réseaux bayésiens permettent à un agent de mettre à jour ses croyances à mesure qu'il observe le monde. À la fin de S4, Dr. Ndoye savait calculer :
...
::: {.callout-note .encadre-prereqbox title="Ce qu'on sait faire après S4 --- et ce qui manque"}
...
::: {.callout-note .encadre-pontbox title="Fil conducteur S4 $\to$ S5"}
| **Séance 4** |  | **Séance 5** |
| --- | --- | --- |
...
::: {.callout-warning .encadre-attentionbox title="Lien direct avec les réseaux bayésiens de S4"}
...
::: {.callout-tip .encadre-exemplebox title="Lien profond avec la mise à jour bayésienne de S4"}
...
C'est une **moyenne exponentielle pondérée** entre l'ancienne croyance et la nouvelle observation. Comparez avec Bayes (S4) :
...
1. **Le MEU est le pont direct entre Bayes (S4) et la décision.** L'agent choisit l'action qui maximise $\sum_s P(s \mid \mathbf{e}) \cdot U(s)$ : les probabilités bayésiennes pèsent chaque conséquence par son utilité. La décision ponctuelle rationnelle est toujours un MEU.
...
5. **Q-learning apprend sans modèle par essai-erreur.** L'erreur TD $\delta_t$ guide la mise à jour de $Q(s,a)$, comme Bayes guidait la mise à jour de $P(H \mid E)$ en S4. C'est de la descente de gradient sur un tableau.
