...
{{< svg ../figures/f10_neurone_detail.svg alt="Un neurone à trois entrées. Les valeurs x_j (bleu) sont pondérées par les poids w_j (vert, sur les flèches) ; on ajoute le biais b (rouge) ; la somme donne z ; l'activation f produit la sortie a. Ici z = 0,5 et a = sigma(0,5) = 0,62. Tout le calcul d'un neurone tient dans cette image." largeur="86%" >}}
...
{{< svg ../figures/f10_couche_largeur.svg alt="Quatre entrées alimentent trois neurones (la largeur de la couche). Chaque neurone a ses propres poids vers les quatre entrées — d'où 4 fois 3 = 12 poids, plus 3 biais. Chaque neurone produit sa sortie a_j. La couche transforme 4 nombres en 3 nombres." largeur="80%" >}}
...
{{< svg ../figures/f10_matrice_W.svg alt="La matrice W (3 fois 4) : en bleu les poids positifs, en rouge les négatifs, en gris les nuls. À droite, le vecteur d'entrée x et le vecteur de biais b. Le produit Wx + b calcule les trois sorties z = (-1; 4; 5,5) d'un seul coup. C'est le pendant, pour les poids, de la matrice X de la Séance 3." largeur="66%" >}}
...
{{< svg ../figures/f10_profondeur.svg alt="Un réseau 4-3-1 : quatre entrées, une couche cachée de trois neurones, une sortie. Chaque couche a sa matrice de poids : W^[1] (3 fois 4) puis W^[2] (1 fois 3). La profondeur est le nombre de couches (ici 2) ; la largeur de la couche cachée est 3." largeur="80%" >}}
...
{{< svg ../figures/f10_params.svg alt="Couche cachée : W^[1] a 3 fois 4 = 12 poids, plus 3 biais = 15. Couche de sortie : W^[2] a 1 fois 3 = 3 poids, plus 1 biais = 4. Total : 19 paramètres. Pour un si petit réseau, déjà 19 nombres à apprendre — d'où l'explosion dès qu'on agrandit, et le besoin d'une méthode automatique d'ajustement." largeur="62%" >}}
...
C'est une *unique* couche linéaire. Vérifié : avec $W^{[1]} = \left(\begin{smallmatrix} 2 & 1\\ 0 & 3 \end{smallmatrix}\right)$, $W^{[2]} = \left(\begin{smallmatrix} 1 & -2 \end{smallmatrix}\right)$, $\mathbf{x} = (1; 2)$, les deux couches donnent $-4{,}5$, exactement comme la couche équivalente ($W_{\text{eq}} = \left(\begin{smallmatrix} 2 & -5 \end{smallmatrix}\right)$). **Empiler du linéaire n'apporte rien** : il faut une activation non-linéaire entre les couches.
:::

## Les activations et leurs dérivées

{{< svg ../figures/f10_activations.svg alt="À gauche, trois activations : sigmoïde (0 à 1), tanh (-1 à 1), ReLU (max(0,z)). À droite, leurs dérivées, qui interviendront dans la rétropropagation : sigma' = a(1-a) (plafonne à 0,25), ' = 1 - a^2, ReLU' = 1 si z > 0 sinon 0. La faible dérivée de la sigmoïde explique qu'elle « écrase » les gradients en profondeur." largeur="86%" >}}
...
{{< svg ../figures/f10_backprop.svg alt="En vert, la propagation avant : l'information va de l'entrée vers la sortie pour produire y. En rouge, la rétropropagation : l'erreur delta remonte de la sortie vers l'entrée, chaque couche ajustant ses poids. Les deux passes s'enchaînent à chaque itération de la descente de gradient." largeur="82%" >}}
