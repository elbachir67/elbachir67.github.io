...
$$ d(\vx, \vx') = \sqrt{\sum_{j=1}^{d}(x_j - x'_j)^2} $$
...
Pour $k$ centres $\vc_1, \dots, \vc_k$. **Affectation** : chaque point $\vx_i$ rejoint le cluster du centre le plus proche. **Mise à jour** : chaque centre devient la moyenne des points de son cluster.
:::

$$ \text{affectation} : \; a_i = \arg\min_{k} \; \lVert \vx_i - \vc_k \rVert^2 \qquad \text{mise à jour} : \; \vc_k = \frac{1}{|C_k|}\sum_{i \in C_k} \vx_i $$

La première équation attribue à chaque point l'indice $a_i$ du centre le plus proche (au sens de la distance au carré). La seconde recalcule chaque centre $\vc_k$ comme le barycentre (la moyenne) des points $C_k$ qui lui sont affectés. On boucle entre les deux jusqu'à ce que les affectations ne changent plus. La répétition de ces deux gestes — proche, moyenne — *est* l'algorithme.

## k-means à la main — 6 points, $k=2$ (1/2)

::: {.callout-tip .ucad-ex title="Affectation puis mise à jour"}
Six points : $(1,1), (2,1), (1,2)$ en bas à gauche, $(5,6), (6,5), (6,6)$ en haut à droite. On vise $k = 2$ clusters. Initialisation *volontairement médiocre* : $\vc_1 = (1,1)$, $\vc_2 = (2,1)$ (les deux centres collés en bas).

- **Affectation 1** : chaque point au centre le plus proche $\to$ labels $[0,1,0,1,1,1]$ (les trois points du haut rejoignent $\vc_2$, plus proche). Inertie $= 108$.
- **Mise à jour 1** : $\vc_1 = $ moyenne de $\{(1,1),(1,2)\} = (1;\ 1{,}5)$ ; $\vc_2 = $ moyenne des quatre autres $= (4{,}75;\ 4{,}5)$.
...
- **Mise à jour 2** : $\vc_1 = (1{,}33;\ 1{,}33)$, $\vc_2 = (5{,}67;\ 5{,}67)$.
...
$$ \mathcal{I} = \sum_{k=1}^{K}\sum_{i \in C_k} \lVert \vx_i - \vc_k \rVert^2 $$
...
{{< svg ../figures/f9_eigen.svg alt="Un nuage de points de covariance psmallmatrix 4 & 2 \ 2 & 4 psmallmatrix. Le vecteur propre principal (rouge, = 6) pointe la direction où le nuage s'étire le plus — la diagonale. Le second (bleu, = 2), perpendiculaire, capte l'étalement résiduel. Les vecteurs propres de la covariance sont les axes naturels des données." largeur="52%" >}}
